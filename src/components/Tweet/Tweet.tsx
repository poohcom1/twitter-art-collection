...
function TweetSkeleton(props: SkeletonProps) {
  return <Skeleton style={{ opacity: "30%" }} {...props} />;
}
...
export default function Tweet(props: {
  ...
  const onImageClick = useCallback(
    (index: number) => {
      if (data) window.open(`${data.url}/photo/${index + 1}`, "_blank");
    },
    [data]
  );
    ...
    <div className={`tweet ${props.darkMode ? "dark" : ""}`}>
      <div className="tweet__header">
        <a
          ...
          href={data && `https://twitter.com/${data?.username}`}
          target="_blank"
          rel="noreferrer"
          tabIndex={-1}
        >
        ...
        >
          <h3 className={`tweet__header-name ${props.darkMode ? "dark" : ""}`}>
            {data?.name || <TweetSkeleton width="100px" />}
          </h3>
          <p className="tweet__header-username">
            {data ? "@" + data.username : <TweetSkeleton width="80px" />}
          </p>
        </a>
      ...
      <div className="tweet__content">
        <div className={`tweet__content-text ${props.darkMode ? "dark" : ""}`}>
          {data ? (
            data.content.text ? (
              injectTweetLink(data.content.text)
            ) : (
              ""
            )
          ) : (
            <TweetSkeleton count={1} height="12px" />
          )}
        </div>
        {data ? (
          <div
