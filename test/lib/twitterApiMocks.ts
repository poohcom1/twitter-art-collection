...
class MockTweetPaginator implements Partial<TweetV2UserLikedTweetsPaginator> {
    ...
    async fetchNext(maxResults: number | undefined): Promise<TweetV2UserLikedTweetsPaginator> {
        if (maxResults) {
...
export class MockTwitterApi implements Partial<TwitterApiv2ReadOnly> {
    ...
    userLikedTweets = async (
        ...
        return new MockTweetPaginator(this.likedTweetIds, this.resultCount) as unknown as TweetV2UserLikedTweetsPaginator
    };
}
